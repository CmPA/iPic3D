...
int Collective::ReadRestart(string inputfile) {
  ...
  string numGrid_Str= ss1.str();


  // Open the setting file for the restart.
  file_id = H5Fopen((inputfile + "/settings_G"+ numGrid_Str +  ".hdf").c_str(), H5F_ACC_RDWR, H5P_DEFAULT); 
  if (file_id < 0) {
    cout << "couldn't open file: " << inputfile << endl;
    return -1;
  }

  // all this info is already acquired in ReadInput
  /*
  // read c
  ...
  */
  ...
  file_id = H5Fopen((inputfile + "/restart0_G"+ numGrid_Str + ".hdf").c_str(), H5F_ACC_RDWR, H5P_DEFAULT);
  if (file_id < 0) {
    cout << "couldn't open file: " << inputfile << endl;
    return -1;
  }
