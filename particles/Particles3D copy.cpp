...
int Particles3D::mover_PC(Grid* grid,VirtualTopology3D* vct, Field* EMf){
	...
	for (i=0; i <  (nop-(P_SAME_TIME-1)); i+=P_SAME_TIME){ 
		...
		for(int innter=0; innter < NiterMover; innter++){	  
			...
			//if (ix[0] < 1){cout << " - X  " << i << " 0 " << ix[0] << endl; ix[0] = 1;}
		    //if (iy[0] < 1){cout << " - Y"   << i << " 0 " << iy[0] << endl; ix[0] = 1;}
		    //if (iz[0] < 1){cout << " - Z" << i << " 0 " << iz[0] << endl; ix[0] = 1;}
		    //if (ix[0] > (nxn-1)){cout << " - X" << i << " 0 " << ix[0] << endl; ix[0] = 1;}
		    //if (iy[0]> (nyn-1)){cout << " _ Y" << endl; iy[0] = nyn-1;}
		    //if (iz[0] > (nzn-1)){cout << " _ Z" << endl; iz[0] = nzn-1;}
			//if (ix[1] < 1){cout << " - X 1" << " index p" << i+1 << "over np " << nop << endl; ix[1] = 1;}
		    //if (iy[1] < 1){cout << " - Y 1" << " index p" << i+1 << "over np " << nop << endl; iy[1] = 1;}
		    //if (iz[1] < 1){cout << " - Z 1" << " index p" << i+1 << "over np " << nop << endl; iz[1] = 1;}
		    //if (ix[1] > (nxn-1)){cout << " - X 1" << " index p" << i+1 <<  "over np " << nop << endl; ix[1] = nxn-1;}
		    //if (iy[1]> (nyn-1)){cout << " _ Y 1" << " index p" << i+1 <<  "over np " << nop << endl; iy[1] = nyn-1;}
		    //if (iz[1] > (nzn-1)){cout << " _ Z 1" << " index p" << i+1 <<  "over np " << nop << endl; iz[1] = nzn-1;}
		    // CALCULATE WEIGHTS
            for(int p=0; p < P_SAME_TIME; p++){
