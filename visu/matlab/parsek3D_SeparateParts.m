...
%			  
%   parsek3D reads data saved in hdf5 from all processors and assemble them in the 
...
global UEB_0 REB_0
global B1x B1y B1z Bx0 By0 Bz0
global Nxc Nyc Nzc


results_dir='/Users/innocent/Documents/Aurora/Aurora_Max_Lx10_2D_Longer_LessPPC/'
results_dir='/Users/innocent/Documents/ExpandingBox/iPic3D/testAlfven/'
results_dir='/Users/innocent/Documents/ExpandingBox/iPic3D/test_separateParts/'
%variable_list='B E rho J pressure';
%variable_list='B E rho J ' % pressure'; 
variable_list='x v q'; 
%variable_list='B E rho J pressure'; % list of variable you want to load :
                                                  %  B, E -> magnetic and electric field
                                                  %  A, phi -> potentials
                                                  %  J, rho -> current and number densities
                                                  %  pressure
...
%% when fields and particles are separated
PART=1

setting_name=[results_dir 'settings.hdf']; % file with collective settings
%% this for the topology
processor_name_TOP=[results_dir 'proc']; % file with data for each processor (i.e. proc0, proc1, ...)
if PART ==0
    processor_name=[results_dir 'proc']; % file with data for each processor (i.e. proc0, proc1, ...)
else
    processor_name=[results_dir 'procPart']; % file with data for each processor (i.e. proc0, proc1, ...)
end
...
for i=1:Nprocs
 cart=hdf5read([processor_name_TOP,num2str(i-1),'.hdf'],'/topology/cartesian_coord');
 MAP(cart(1)+1,cart(2)+1,cart(3)+1)=hdf5read([processor_name_TOP,num2str(i-1),'.hdf'],'/topology/cartesian_rank')+1;
end
    ...
    for ii=1:nnGroups
       ...
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/Bx_ext') & strfind (variable_list, 'B')
           ...
           for time=1:ntime
            ...
            Bx_ext_time=[Bx_ext_time;str2double(regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/Bx_ext/cycle_',''))];
           end
           [Bx_ext_time,Bx_ext_index]=intersect(Bx_ext_time,sort(Bx_ext_time),'rows');
       end
       ...
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/By_ext') & strfind (variable_list, 'B')
           ...
           for time=1:ntime
            ...
            By_ext_time=[By_ext_time;str2double(regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/By_ext/cycle_',''))];
           end
           [By_ext_time,By_ext_index]=intersect(By_ext_time,sort(By_ext_time),'rows');
       end
       ...
       if strcmp(info.GroupHierarchy.Groups(i).Groups(ii).Name,'/fields/Bz_ext') & strfind (variable_list, 'B')
           ...
           for time=1:ntime
            ...
            Bz_ext_time=[Bz_ext_time;str2double(regexprep(info.GroupHierarchy.Groups(i).Groups(ii).Datasets(time).Name,'/fields/Bz_ext/cycle_',''))];
           end
           [Bz_ext_time,Bz_ext_index]=intersect(Bz_ext_time,sort(Bz_ext_time),'rows');
       end
       %%% end external fields
...
if exist('Bx_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Bx_row=cat(1,Bx_row,Bx_slab(:,:,:,:,MAP(i,j,k)));
		  end
          Bx_col=cat(2,Bx_col,Bx_row);
          Bx_row=[];
          end
		  Bx=cat(3,Bx,Bx_col);
		  Bx_col=[];
...
if exist('By_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                By_row=cat(1,By_row,By_slab(:,:,:,:,MAP(i,j,k)));
        end
        By_col=cat(2,By_col,By_row);
        By_row=[];
      end
	  By=cat(3,By,By_col);
	  By_col=[];  
...
if exist('Bz_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Bz_row=cat(1,Bz_row,Bz_slab(:,:,:,:,MAP(i,j,k)));
        end
         Bz_col=cat(2,Bz_col,Bz_row);
        Bz_row=[];
      end
	  Bz=cat(3,Bz,Bz_col);
	  Bz_col=[];  
...
%Bx
if exist('Bx_ext_slab','var')
    Bx_ext_slab=(Bx_ext_slab(1:end-1,:,:,:,:)+Bx_ext_slab(2:end,:,:,:,:))*0.5;
    Bx_ext_slab=(Bx_ext_slab(:,1:end-1,:,:,:)+Bx_ext_slab(:,2:end,:,:,:))*0.5;
	Bx_ext_slab=(Bx_ext_slab(:,:,1:end-1,:,:)+Bx_ext_slab(:,:,2:end,:,:))*0.5;
    Bx_ext=[];Bx_ext_row=[]; Bx_ext_col=[];
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Bx_ext_row=cat(1,Bx_ext_row,Bx_ext_slab(:,:,:,:,MAP(i,j,k)));
		  end
          Bx_ext_col=cat(2,Bx_ext_col,Bx_ext_row);
          Bx_ext_row=[];
          end
		  Bx_ext=cat(3,Bx_ext,Bx_ext_col);
		  Bx_ext_col=[];
		end
		 
    clear Bx_ext_row Bx_ext_col Bx_ext_slab

 for h=1:size(Bx_ext_index)
    BBx_ext(:,:,:,h)=Bx_ext(:,:,:,Bx_ext_index(h));
 end
Bx_ext=BBx_ext; clear BBx_ext Bx_ext_index
Bx_ext=squeeze(Bx_ext);
end %endif

%By_ext
if exist('By_ext_slab','var')
    By_ext_slab=(By_ext_slab(1:end-1,:,:,:,:)+By_ext_slab(2:end,:,:,:,:))*0.5;
    By_ext_slab=(By_ext_slab(:,1:end-1,:,:,:)+By_ext_slab(:,2:end,:,:,:))*0.5;
	By_ext_slab=(By_ext_slab(:,:,1:end-1,:,:)+By_ext_slab(:,:,2:end,:,:))*0.5;
    By_ext=[];By_ext_row=[]; By_ext_col=[];
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                By_ext_row=cat(1,By_ext_row,By_ext_slab(:,:,:,:,MAP(i,j,k)));
		  end
          By_ext_col=cat(2,By_ext_col,By_ext_row);
          By_ext_row=[];
          end
		  By_ext=cat(3,By_ext,By_ext_col);
		  By_ext_col=[];
		end
		 
    clear By_ext_row By_ext_col By_ext_slab

 for h=1:size(By_ext_index)
    BBy_ext(:,:,:,h)=By_ext(:,:,:,By_ext_index(h));
 end
By_ext=BBy_ext; clear BBy_ext By_ext_index
By_ext=squeeze(By_ext);
end %endif

%Bz_ext
if exist('Bz_ext_slab','var')
    Bz_ext_slab=(Bz_ext_slab(1:end-1,:,:,:,:)+Bz_ext_slab(2:end,:,:,:,:))*0.5;
    Bz_ext_slab=(Bz_ext_slab(:,1:end-1,:,:,:)+Bz_ext_slab(:,2:end,:,:,:))*0.5;
	Bz_ext_slab=(Bz_ext_slab(:,:,1:end-1,:,:)+Bz_ext_slab(:,:,2:end,:,:))*0.5;
    Bz_ext=[];Bz_ext_row=[]; Bz_ext_col=[];
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Bz_ext_row=cat(1,Bz_ext_row,Bz_ext_slab(:,:,:,:,MAP(i,j,k)));
		  end
          Bz_ext_col=cat(2,Bz_ext_col,Bz_ext_row);
          Bz_ext_row=[];
          end
		  Bz_ext=cat(3,Bz_ext,Bz_ext_col);
		  Bz_ext_col=[];
		end
		 
    clear Bz_ext_row Bz_ext_col Bz_ext_slab

 for h=1:size(Bz_ext_index)
    BBz_ext(:,:,:,h)=Bz_ext(:,:,:,Bz_ext_index(h));
 end
Bz_ext=BBz_ext; clear BBz_ext Bz_ext_index
Bz_ext=squeeze(Bz_ext);
end %endif
...
if exist('Ex_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Ex_row=cat(1,Ex_row,Ex_slab(:,:,:,:,MAP(i,j,k)));
        end
        Ex_col=cat(2,Ex_col,Ex_row);
        Ex_row=[];
      end
	  Ex=cat(3,Ex,Ex_col);
	  Ex_col=[];  
...
if exist('Ey_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Ey_row=cat(1,Ey_row,Ey_slab(:,:,:,:,MAP(i,j,k)));
        end
        Ey_col=cat(2,Ey_col,Ey_row);
        Ey_row=[];
      end
	  Ey=cat(3,Ey,Ey_col);
	  Ey_col=[];  
...
if exist('Ez_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Ez_row=cat(1,Ez_row,Ez_slab(:,:,:,:,MAP(i,j,k)));
        end
        Ez_col=cat(2,Ez_col,Ez_row);
        Ez_row=[];
      end
	  Ez=cat(3,Ez,Ez_col);
	  Ez_col=[];  
...
if exist('phi_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                phi_row=cat(1,phi_row,phi_slab(:,:,:,:,MAP(i,j,k)));
        end
       phi_col=cat(2,phi_col,phi_row);
	   phi_row=[];
      end
...
if exist('Jx_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Jx_row=cat(1,Jx_row,Jx_slab(:,:,:,:,MAP(i,j,k)));
        end
       Jx_col=cat(2,Jx_col,Jx_row);
	   Jx_row=[];
      end
	  Jx=cat(3,Jx,Jx_col);
	  Jx_col=[];  
...
if exist('Jy_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Jy_row=cat(1,Jy_row,Jy_slab(:,:,:,:,MAP(i,j,k)));
        end
        Jy_col=cat(2,Jy_col,Jy_row);
	   Jy_row=[];
      end
	  Jy=cat(3,Jy,Jy_col);
	  Jy_col=[];  
...
if exist('Jz_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                Jz_row=cat(1,Jz_row,Jz_slab(:,:,:,:,MAP(i,j,k)));
        end
       Jz_col=cat(2,Jz_col,Jz_row);
	   Jz_row=[];
      end
	  Jz=cat(3,Jz,Jz_col);
	  Jz_col=[];  
...
if exist('rho_slab','var')
    ...
    for i=1:XLEN
        for j=1:YLEN
		  for k=1:ZLEN
                rho_row=cat(1,rho_row,rho_slab(:,:,:,:,MAP(i,j,k)));
        end
        rho_col=cat(2,rho_col,rho_row);
	    rho_row=[];
      end
   ...
   if exist(['k_nrg_slab' num2str(nspec-1)],'var')
    ...
    if exist(['Jxs_slab' num2str(nspec-1)],'var')
        ...
        for i=1:XLEN
            for j=1:YLEN
			  for k=1:ZLEN
                    Jxs_row=cat(1,Jxs_row,Jxs_slab(:,:,:,:,MAP(i,j,k)));
            end
            Jxs_col=cat(2,Jxs_col,Jxs_row);
	        Jxs_row=[];
      end
    ...
    if exist(['Jys_slab' num2str(nspec-1)],'var')
        ...
        for i=1:XLEN
            for j=1:YLEN
			  for k=1:ZLEN
                    Jys_row=cat(1,Jys_row,Jys_slab(:,:,:,:,MAP(i,j,k)));
            end
             Jys_col=cat(2,Jys_col,Jys_row);
	        Jys_row=[];
      end
     ...
     if exist(['Jzs_slab' num2str(nspec-1)],'var')
         ...
         for i=1:XLEN
             for j=1:YLEN
			   for k=1:ZLEN
                    Jzs_row=cat(1,Jzs_row,Jzs_slab(:,:,:,:,MAP(i,j,k)));
            end
			Jzs_col=cat(2,Jzs_col,Jzs_row);
	        Jzs_row=[]; 
      end
...
%return   
	...
	rhos_slab=(rhos_slab(:,:,1:end-1,:,:)+rhos_slab(:,:,2:end,:,:))*0.5;
        ...
        for i=1:XLEN
            for j=1:YLEN
			  for k=1:ZLEN
                    rhos_row=cat(1,rhos_row,rhos_slab(:,:,:,:,MAP(i,j,k)));
            end
            rhos_col=cat(2,rhos_col,rhos_row);
	        rhos_row=[]; 
      end
 ...
 %pxx
   if exist(['pxx_slab' num2str(nspec-1)],'var')
       ...
       pxx=[];pxx_row=[];pxx_row=[]; pxx_col=[];
        for i=1:XLEN
            for j=1:YLEN
			  for k=1:ZLEN
                    pxx_row=cat(1,pxx_row,pxx_slab(:,:,:,:,MAP(i,j,k)));
            end
            pxx_col=cat(2,pxx_col,pxx_row);
	        pxx_row=[]; 
      end
   ...
   if exist(['pxy_slab' num2str(nspec-1)],'var')
       ...
       pxy=[];pxy_row=[]; pxy_col=[];
        for i=1:XLEN
            for j=1:YLEN
			  for k=1:ZLEN
                    pxy_row=cat(1,pxy_row,pxy_slab(:,:,:,:,MAP(i,j,k)));
            end
            pxy_col=cat(2,pxy_col,pxy_row);
	        pxy_row=[]; 
      end
   ...
   if exist(['pxz_slab' num2str(nspec-1)],'var')
        ...
        for i=1:XLEN
            for j=1:YLEN
			  for k=1:ZLEN
                    pxz_row=cat(1,pxz_row,pxz_slab(:,:,:,:,MAP(i,j,k)));
            end
			pxz_col=cat(2,pxz_col,pxz_row);
	        pxz_row=[]; 
      end
  ...
  if exist(['pyz_slab' num2str(nspec-1)],'var')
        ...
        for i=1:XLEN
            for j=1:YLEN
			  for k=1:ZLEN
                    pyz_row=cat(1,pyz_row,pyz_slab(:,:,:,:,MAP(i,j,k)));
            end
           pyz_col=cat(2,pyz_col,pyz_row);
	        pyz_row=[]; 
      end
  ...
  if exist(['pyy_slab' num2str(nspec-1)],'var')
        ...
        for i=1:XLEN
            for j=1:YLEN
			  for k=1:ZLEN
                    pyy_row=cat(1,pyy_row,pyy_slab(:,:,:,:,MAP(i,j,k)));
            end
            pyy_col=cat(2,pyy_col,pyy_row);
	        pyy_row=[]; 
      end
...
%             for j=1:YLEN
% 			  for k=1:ZLEN
%                     pzz_row=cat(1,pzz_row,pzz_slab(:,:,:,:,MAP(i,j,k)));
%             end
%             pzz_col=cat(2,pzz_col,pzz_row);
% 	        pzz_row=[]; 
%       end
...
%   end
  ...
  if exist(['pzz_slab' num2str(nspec-1)],'var')
      ...
      pzz_col=[];  
        ...
        clear pzz_row pzz_col pzz_slab
        for h=1:size(p_index)
          Bpzz(:,:,:,h)=pzz(:,:,:,p_index(h));
        end
        pzz=Bpzz; clear Bpzz
...
display('grouping species')
%comment when tracking
%%% group the tracking species
if (exist('x2') && (qom(1)== qom(3)))
    x0=vertcat(x0, x2);
    y0=vertcat(y0, y2);
    z0=vertcat(z0, z2);
...
display('saving')
if PART ==0
    save ([results_dir 'workspace.mat'], '-v7.3')
else
    save ([results_dir 'workspacePart.mat'], '-v7.3')
end

%save ([results_dir 'workspace2.mat'])