...
directory= '/scratch/leuven/300/vsc30068/MC/ReducedC/MRX_NoColl_OI/'
...
data_read = {'Fields': 0, 'Currents': 0, 'Rho': 0, 'Pressure': 0, 'Inertia': 0, 'Inertia_SC': 1, 'MC': 0, 'InertiaIons':0, 'Inertia_SCIons':1} 
# Which cycles to process
first_cycle = 0
last_cycle = 15000 #8000 #1000 #800 #1900 ##138 #2000 # 200 #1200
step =  400 #100 #100
...
str_inertiaO = 'inertiaE' ## OHM specific
str_inertiaIons = 'inertia' ## OHM specific 
str_inertiaIonsO = 'inertiaI' ## OHM specific 
str_MC= 'MC'
str_inertia_SC = 'inertia' ## OHM specific
str_inertia_SCO = 'inertiaSCE' ## OHM specific 
str_inertiaIons_SC = 'inertia' ## OHM specific
str_inertiaIons_SCO = 'inertiaSCI' ## OHM specific 
...
inertiaIons_comp_names = ['Ji_MA_x', 'Ji_MA_y', 'Ji_MA_z', 'T1_MA_I_x', 'T1_MA_I_y', 'T1_MA_I_z', 'T2_MA_I_x', 'T2_MA_I_y', 'T2_MA_I_z', 'T3_MA_I_x', 'T3_MA_I_y', 'T3_MA_I_z', 'T4_MA_I_x', 'T4_MA_I_y', 'T4_MA_I_z'] ## OHM specific
inertia_SC_comp_names = ['rhoe_MA']
inertiaIons_SC_comp_names = ['rhoi_MA']
...
inertiaIons_comp_count = len(inertiaIons_comp_names)
inertiaIons_SC_comp_count = len(inertiaIons_SC_comp_names)
field_vec_names = ['B', 'E']
current_vec_name = 'J'
...
inertia_SC_vec_names = ['rhoe_MA']
inertiaIons_vec_names = [ 'Ji_MA', 'T1_MA_I', 'T2_MA_I', 'T3_MA_I', 'T4_MA_I'] ## NB: i still have to extract rhoe_MA
inertiaIons_SC_vec_names = ['rhoi_MA']
...
if len(list_read_proc) > 0:
    ...
    if data_read['Inertia']: 
        ...
        for cycle in list_cycle:  
            ...
            writeVTR(os.path.join(out_directory, str_inertiaO + repr(cycle).rjust(8,"0") + "-" + repr(mpi_myrank) + ".vtr"), {}, vector_inertia,
                     vtkXcoordinates, vtkYcoordinates, vtkZcoordinates, RangeYlocal)
            if mpi_myrank == 0:
                pvtr_inertia_list.append(str_inertiaO + repr(cycle).rjust(8,"0") + ".pvtr")
                writePVTR(os.path.join(out_directory, pvtr_inertia_list[-1]), cycle, str_inertiaO, [], vector_inertia.keys(), Nxc, Nyc, Nzc, numproc)

        if mpi_myrank == 0:
            writePVD(os.path.join(out_directory, str_inertiaO+ '.pvd'), pvtr_inertia_list, list_cycle)
            print 'Inertia written'
    
    ## scalar components for Ohm-related things
    """ 5. Inertia: Ohm's law component """
    if data_read['Inertia_SC']: 
        ...
        for cycle in list_cycle:    
            ...
            for proc_filename in list_proc_files:
                h5proc_file, bounds = openProcFile(proc_filename, list_read_proc[0], shape)
                ## Cycle over field component
                for c in range(inertia_SC_comp_count):
                    for node in h5proc_file.walk_nodes('/' + str_inertia_SC + '/' + inertia_SC_comp_names[c]):
                        if type(node) == tables.array.Array:
                            num = int(node.name[6:])
                            if num == cycle:
                                Fields_local[c, bounds[0]:bounds[1], bounds[2]:bounds[3], bounds[4]:bounds[5]] = node.read()
            ...
            vector_inertia_SC = {} 
            for c in range(len(inertia_SC_vec_names)): 
                vector_inertia_SC[inertia_SC_vec_names[c]] = Fields_local[c, :, :, :]
                
            
                
            writeVTR(os.path.join(out_directory, str_inertia_SCO + repr(cycle).rjust(8,"0") + "-" + repr(mpi_myrank) + ".vtr"), vector_inertia_SC, {},
                     vtkXcoordinates, vtkYcoordinates, vtkZcoordinates, RangeYlocal)
            if mpi_myrank == 0:
                pvtr_inertia_SC_list.append(str_inertia_SCO + repr(cycle).rjust(8,"0") + ".pvtr")
                writePVTR(os.path.join(out_directory, pvtr_inertia_SC_list[-1]), cycle, str_inertia_SCO,  vector_inertia_SC.keys(), [], Nxc, Nyc, Nzc, numproc)


        if mpi_myrank == 0:
            writePVD(os.path.join(out_directory, str_inertia_SCO+ '.pvd'), pvtr_inertia_SC_list, list_cycle)
    ...
    ### here, Ohm's law for ions
    """ 5. Inertia: Ohm's law componen for Ionst """
    if data_read['InertiaIons']: 
        if mpi_myrank == 0: pvtr_inertiaIons_list = []
        for cycle in list_cycle:  
            Fields_local = numpy.zeros((inertiaIons_comp_count,Nxc+1,Nylocal+1,Nzc+1), dtype="float32")
            for proc_filename in list_proc_files:
                h5proc_file, bounds = openProcFile(proc_filename, list_read_proc[0], shape)
                ## Cycle over field components
                for c in range(inertiaIons_comp_count):
                    for node in h5proc_file.walk_nodes('/' + str_inertiaIons + '/'+ inertiaIons_comp_names[c]):
                        if type(node) == tables.array.Array:
                            num = int(node.name[6:])
                            if num == cycle:
                                Fields_local[c, bounds[0]:bounds[1], bounds[2]:bounds[3], bounds[4]:bounds[5]] = node.read()
            ...
            vector_inertiaIons = {}
            for c in range(len(inertiaIons_vec_names)): 
                vector_inertiaIons[inertiaIons_vec_names[c]] = Fields_local[3*c:3*(c+1), :, :, :]
            writeVTR(os.path.join(out_directory, str_inertiaIonsO + repr(cycle).rjust(8,"0") + "-" + repr(mpi_myrank) + ".vtr"), {}, vector_inertiaIons,
                     vtkXcoordinates, vtkYcoordinates, vtkZcoordinates, RangeYlocal)
            if mpi_myrank == 0:
                pvtr_inertiaIons_list.append(str_inertiaIonsO + repr(cycle).rjust(8,"0") + ".pvtr")
                writePVTR(os.path.join(out_directory, pvtr_inertiaIons_list[-1]), cycle, str_inertiaIonsO, [], vector_inertiaIons.keys(), Nxc, Nyc, Nzc, numproc)

        if mpi_myrank == 0:
            writePVD(os.path.join(out_directory, str_inertiaIonsO+ '.pvd'), pvtr_inertiaIons_list, list_cycle)
            print 'Inertia IONS written'
    
    ## scalar components for Ohm-related things
    """ 5. Inertia: Ohm's law component """
    if data_read['Inertia_SCIons']: 
        if mpi_myrank == 0: pvtr_inertiaIons_SC_list = []
        for cycle in list_cycle:    
            Fields_local = numpy.zeros((inertiaIons_SC_comp_count,Nxc+1,Nylocal+1,Nzc+1), dtype="float32")            
            for proc_filename in list_proc_files:
                h5proc_file, bounds = openProcFile(proc_filename, list_read_proc[0], shape)
                ## Cycle over field component
                for c in range(inertiaIons_SC_comp_count):
                    for node in h5proc_file.walk_nodes('/' + str_inertiaIons_SC + '/' + inertiaIons_SC_comp_names[c]):
                        if type(node) == tables.array.Array:
                            num = int(node.name[6:])
                            if num == cycle:
                                Fields_local[c, bounds[0]:bounds[1], bounds[2]:bounds[3], bounds[4]:bounds[5]] = node.read()
            ...
            vector_inertiaIons_SC = {} 
            for c in range(len(inertiaIons_SC_vec_names)): 
                vector_inertiaIons_SC[inertiaIons_SC_vec_names[c]] = Fields_local[c, :, :, :]
                
            
                
            writeVTR(os.path.join(out_directory, str_inertiaIons_SCO + repr(cycle).rjust(8,"0") + "-" + repr(mpi_myrank) + ".vtr"), vector_inertiaIons_SC, {},
                     vtkXcoordinates, vtkYcoordinates, vtkZcoordinates, RangeYlocal)
            if mpi_myrank == 0:
                pvtr_inertiaIons_SC_list.append(str_inertiaIons_SCO + repr(cycle).rjust(8,"0") + ".pvtr")
                writePVTR(os.path.join(out_directory, pvtr_inertiaIons_SC_list[-1]), cycle, str_inertiaIons_SCO,  vector_inertiaIons_SC.keys(), [], Nxc, Nyc, Nzc, numproc)


        if mpi_myrank == 0:
            writePVD(os.path.join(out_directory, str_inertiaIons_SCO+ '.pvd'), pvtr_inertiaIons_SC_list, list_cycle)
        
            print 'Inertia Ions SC written'
    ### end Ohm's law for ions
