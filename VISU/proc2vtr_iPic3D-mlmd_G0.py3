...
#directory = "/home/mariaelena/iPic3D_MLMD/iPic3D/dataMLMD/"
#out_directory ="/home/mariaelena/iPic3D_MLMD/iPic3D/dataMLMD/data_vtr" + str(GN)
directory ="/home/meinnocenti/iPic3D_MLMD/iPic3D/ParticleTest_PRA2_InterpF_BBC_NPP_1_LONG_OneCore_initTestBC/"
out_directory ="/home/meinnocenti/iPic3D_MLMD/iPic3D/ParticleTest_PRA2_InterpF_BBC_NPP_1_LONG_OneCore_initTestBC/data_vtr" + str(GN)


directory="/Users/innocent/Documents/MLMD/iPic3D/test/"
out_directory="/Users/innocent/Documents/MLMD/iPic3D/test/data_vtr" + str(GN)
...
def writePVTR(pvtr_name, cycle, vtr_prefix, scalar_fields, vector_fields, Nxc, Nyc, Nzc, numproc):
    ...
    for i in range(numproc):
        ...
        piece.setAttribute("Extent", "0 " + str(Nxc) + " 0 " + str(Nyc) + " " + str(int(i*Nzc/numproc)) + " " + str(int((i+1)*Nzc/numproc))) 
...
def openProcFile(proc_filename, read_proc0, shape):
    ...
    #transition from python 2 to 3; if i don;t do that, i get 'UnboundLocalError'
    bounds= [-1, -1, -1, -1, -1, -1];
...
print('setting_filename is '+ setting_filename)
try:
...
if numproc> ZLEN:
    print('ask for ', ZLEN, ' core max for this job, otherwise problems')
    print('I am returning now')
    exit()

#if mpi_myrank> numproc
#    print('I am rank ', mpi_myrank, ' and i only need ', numproc, 'cores, I am exiting')
#    exit()

# Shape of each individual subdomain. We assume that each processor has domain of the same size!
# to transition form python 2 to 3
#shape = (Nxc/XLEN+1, Nyc/YLEN+1, Nzc/ZLEN+1) 
shape = (int(Nxc/XLEN)+1, int(Nyc/YLEN)+1, int(Nzc/ZLEN)+1) 
print('I am rank ', mpi_myrank, ' shape is ', shape)
species = list(range(ns))
...
for Z in Zrange:
    Z= int(Z) # this to transition to python3
        ...
        for cycle in list_cycle:    
            for proc_filename in list_proc_files:
                ...
                for c in range(field_comp_count):
                    # to transition from python 2 to 3
            ...
            if mpi_myrank == 0:
                ...
                writePVTR(os.path.join(out_directory, pvtr_fields_list[-1]), cycle, str_fields, [], list(vector_fields.keys()), Nxc, Nyc, Nzc, numproc)
    ...
    if data_read['Rho']: 
        ...
        for cycle in list_cycle:    
            ...
            if mpi_myrank == 0:            
                ...
                writePVTR(os.path.join(out_directory, pvtr_rho_list[-1]), cycle, str_rho, list(rho_fields.keys()), [], Nxc, Nyc, Nzc, numproc)
    ...
    if data_read['Currents']:
        ...
        for cycle in list_cycle:    
            ...
            if mpi_myrank == 0:
                ...
                writePVTR(os.path.join(out_directory, pvtr_currents_list[-1]), cycle, str_currents, [], list(j_fields.keys()), Nxc, Nyc, Nzc, numproc)
    ...
    if data_read['Pressure']: 
        ...
        for cycle in list_cycle:    
            ...
            if mpi_myrank == 0:            
                ...
                writePVTR(os.path.join(out_directory, pvtr_press_list[-1]), cycle, str_pressure, list(p_fields.keys()), [], Nxc, Nyc, Nzc, numproc)

        if mpi_myrank == 0:
            writePVD(os.path.join(out_directory, str_pressure + '.pvd'), pvtr_press_list, list_cycle)
            print('Pressure written')

#tranistioning from python 2 to 3
#if mpi_myrank == 0: print('Time elapsed: ', time.clock() - start_time)
if mpi_myrank == 0: print('Time elapsed: ', time.process_time() - start_time)
