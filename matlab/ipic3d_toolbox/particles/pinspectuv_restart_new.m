...
q=[q;squeeze(hdf5read(info.GroupHierarchy.Groups(3).Groups(is+2).Groups(1).Datasets(it)))];
x=[x;squeeze(hdf5read(info.GroupHierarchy.Groups(3).Groups(is+2).Groups(5).Datasets(it)))];
y=[y;squeeze(hdf5read(info.GroupHierarchy.Groups(3).Groups(is+2).Groups(6).Datasets(it)))];
z=[z;squeeze(hdf5read(info.GroupHierarchy.Groups(3).Groups(is+2).Groups(7).Datasets(it)))];
...
u=[u;squeeze(hdf5read(info.GroupHierarchy.Groups(3).Groups(is+2).Groups(2).Datasets(it)))];
v=[v;squeeze(hdf5read(info.GroupHierarchy.Groups(3).Groups(is+2).Groups(3).Datasets(it)))];
w=[w;squeeze(hdf5read(info.GroupHierarchy.Groups(3).Groups(is+2).Groups(4).Datasets(it)))];
...
title(['x=' num2str((xmin+xmax)/2) '  y=' num2str((ymin+ymax)/2) '  z=' num2str((zmin+zmax)/2) ])% '  xp=' num2str(mean(x))  '   yp=' num2str(mean(y))],'fontsize',[14])
xlabel('upar','fontsize',[14])
	 ylabel('uperp1','fontsize',[14])
	 colorbar
%set(gcf,'Renderer','zbuffer');
