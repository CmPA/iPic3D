...
dire ='/shared02/gianni/tred60/data1/'
...
euler_bal_th_e = vdivPeint-(divQeint-divQbulkeint); %div Q includes both terms

lagrange_bal_bulk_e = vgradUbulkeint-divQbulkeint+JEeint-vdivPeint;
lagrange_bal_th_e = vgradUtheint+vdivPeint-(divQeint-divQbulkeint); %div Q includes both terms
...
%plot(xxx,smooth(vgradUbulkeint),'c')
plot(xxx,smooth(euler_bal_bulk_e),'b')
plot(xxx,smooth(lagrange_bal_bulk_e),'k')
ylim([-1e-7 2e-7])
grid on
%'v\nabla Ubulk',
legend('J.E','divQbulk','vdiv P','balance-Euler','balance-Lagrange','Location','BestOutside')
...
hold on 
plot(xxx,smooth(vdivPeint),'g')
%plot(xxx,smooth(vgradUtheint),'c')
...
%'v\nabla Uth',
legend('divQin','vdiv P','balance-euler','balance-lagrange','Location','BestOutside')
...
euler_bal_th_i = vdivPiint-(divQiint-divQbulkiint);%div Q includes both terms

lagrange_bal_bulk_i = vgradUbulkiint-vdivPiint-divQbulkiint+JEiint;
lagrange_bal_th_i = vgradUthiint+vdivPiint-(divQiint-divQbulkiint);%div Q includes both terms
...
legend('Ji.E','divQbulk','v div P','balance-euler','balance-lagrange','Location','BestOutside')
...
hold on 
plot(xxx,vdivPiint,'g')
plot(xxx,vgradUthiint,'c')
plot(xxx,euler_bal_th_i,'b')
plot(xxx,lagrange_bal_th_i,'k')
ylim([-3e-6 3e-6])
grid on
title('Thermal Energy Ions')
legend('divQenth','v div P','balance-euler','balance-lagrange','Location','BestOutside')
...
%plot(xxx(round(end/2):end),cumsum(vgradUbulkeint(round(end/2):end)),'c')
...
%plot(xxx(round(end/2):-1:1),cumsum(vgradUbulkeint(round(end/2):-1:1)),'c')
...
title('Bulk Energy Electrons')
%legend('J.E','divQbulk','balance-Euler','balance-Lagrange','Location','BestOutside')
...
%plot(xxx(round(end/2):end),cumsum(vgradUbulkiint(round(end/2):end)),'c')
...
%plot(xxx(round(end/2):-1:1),cumsum(vgradUbulkiint(round(end/2):-1:1)),'c')
...
title('Bulk Energy Ions')
%legend('J.E','divQbulk','vdivP','balance-Euler','balance-Lagrange','Location','BestOutside')
...
title('Thermal Energy Electrons')
%legend('divQenth','balance-euler','balance-lagrange','Location','BestOutside')
...
title('Thermal Energy Electrons')
%legend('divQenth','v\nabla P','balance-euler','balance-lagrange','Location','BestOutside')
