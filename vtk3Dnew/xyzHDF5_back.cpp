...
for (int it = initlevel; it < nlevels+1; it++){
	...
	//Rho by species
    readscalar(it,"/moments/species_2/","rho",  NE);
    readscalar(it,"/moments/species_3/","rho",  NI);
    if (ns >4)readscalar(it,"/moments/species_5/","rho",  NO);
    if (ns >4) addreadscalar(it,"/moments/species_4/","rho",  NE);
    writeVTKscalar_species(it,"rho", NE, NI, NO);
   // writeVTKscalar("rho", "e", VX);

	//Currents species0
    readvect(it,"/moments/species_2/","J",  VX, VY, VZ);
    if (ns >4) addreadvect(it,"/moments/species_4/","J",  VX, VY, VZ);
    ...
    //Currents species1
    readvect(it,"/moments/species_3/","J",  VX, VY, VZ);
    ...
    if (ns >4){
    //Currents species2
    readvect(it,"/moments/species_5/","J",  VX, VY, VZ);
    writeVTKvect(it,"J", "o", VX, VY, VZ);

	//Evaluate and save E+VxB
    ohm(NO, BX, BY, BZ, EX, EY, EZ, VX, VY, VZ, OHMX, OHMY, OHMZ);
    writeVTKvect(it,"EMF", "o", VX, VY, VZ);
    vnonfrozen(NO, BX, BY, BZ, EX, EY, EZ, VX, VY, VZ);
    writeVTKvect(it,"VNF", "o", VX, VY, VZ);
    }
}
