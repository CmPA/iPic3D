...
namespace KVF {
    ...
    typedef enum BufferedDataMajorityE
    { DBUFFER_NIL_MAJOR, DBUFFER_ROW_MAJOR, DBUFFER_COL_MAJOR }
    BufferedDataMajority;
...
*/
    ...
    class SimpleDBuffDescriptor : public VarTarget {
        ...
        SimpleDBuffDescriptor( const SimpleDBuffDescriptor& a ) ///< Copy ctor
            : _num_items(0), _majority(a._majority) {set_dims(a._dims);}

        void clear(void)  { _num_items=0; _dims.clear(); _num_item_elts=1; }
    ...
    /*! \brief Abstract interface class for a collection of data values
      ...
      A DataBuffer contains or acquires data, and can be queried for that
      data.

      Subclasses such as SimpleDataBuffer or TSDataBuffer
      have more specific behaviour.
...
*/
    ...
    /*! \brief Abstract class for a DataBuffer consisting of a
      sequence of rectangular matrices

      In a SimpleDataBuffer the data is organized as an ordered sequence of
      items. Each item is a rectangular matrix of elements, where the
      dimensionality, dimension sizes and majority of all matrix items is the same.
      The element values are accessed in sequential order ordered by item,
      and then element order within each item.

      A SimpleDataBuffer object can be queried for
      - number of items it contains
      - number of elements in each item
      - total number of elements
      - the description of the data arrangement

      A SimpleDataBuffer object can also 
      - be rewound, ie subsequent sequential data access starts from beginning
      - be cleared, ie all data erased
...
*/
    ...
    class DataPtrBuffer : public DataBuffer {
        public:
            ...
            string get_type_srep(void) const { return "DataPtrBuffer"; }
            void diag_print(void) const
            { data->diag_print(); data_ptr_name->diag_print(); };
...
*/
    template<class T> class BasicVectorDataBuffer {
        ...
        {;}
            ...
            /*! \brief Retrieve data from data buffer
              ...
              \param n_elts Number of elements to retrieve.
              \param vvals Reference to vector, in which to store values from data buffer
              \param append If true, then values are appended to end of vvals, otherwise
              vvals is cleared before values are copied.
            ...
            {
                if( !append ) vvals.clear();// clear the receiving vector if append not requested
                if( _pos >= _v.size() ) return false;  // nothing to return
                int i=0;  // number of items packed
                while( i<n_elts && _pos < _v.size() )
                { vvals.push_back( _v[ _pos++ ] ); i++; }
            ...
            { _v.push_back(x);
                ...
                /*! \brief Push supplied values as new elements at end of buffer
                  \note The number of items is recalculated using new total number of
                  elements. */
                ...
                { for(int i=0; i<x.size(); i++)
                    ...
                    /*! \brief Push elements of supplied data buffer as new elements at end of buffer.
                      \note The dimensionality etc. of the matrices are not used. The
                      elements of the supplied data buffer are simply copied*/
                    ...
                    { for(int i=0; i<x._v.size(); i++)
                        ...
                        void set_dims( const vector<int>& d ) { _desc->set_dims( d ); }

                        /*! \brief Set the matrix majority */
                        void set_majority( BufferedDataMajority m ) { _desc->set_majority( m ); }

                        /*! \brief Return total number of elements in buffer */
                        int num_elts(void)  const {return _v.size();}
                        /*! \brief Return total number of elements in buffer */
                        int size(void) { return _v.size(); }

                        /*! \brief Return number of items in buffer */
                        int num_items(void) const {return _desc->num_items();}

                        /*! \brief Return number of elements per item. */
                        int num_item_elts(void) const {return _desc->num_item_elts();}

                        /*! \brief Return copy of i-th value in buffer. */
                        T operator[](int i) const { return _v[i]; }

                        /*! \brief Test item (not just element) is approximately equal.
                          \param item_i Index of item within buffer to be tested.
                          \param a Vector of values corresponding to the elements of the item.
                          If there are insufficient elements in \c a, then it is used cyclically.
                          \param precision Bound for relative precision.

                          \returns Boolean true if  all elements of item \c item_i
                          are approximately equal to corresponding elements of \c a, within
                          given relative precision.
                          */ 
                        bool item_approx_equal( int item_i, const vector<T>& a,
                        ...
                        { bool ok=true;
                            ...
                            {  _v.erase( _v.start() + i*( _desc->num_item_elts() ),
                                    _v.start() + (i+1)*( _desc->num_item_elts() ) ); }
    ...
    // ----------------------------------------------------------------------------
    class NumericVDataBuffer
        : public BasicVectorDataBuffer<double>, public NumericDataBuffer {
            ...
            public:
                ...
                void rewind(void) { BasicVectorDataBuffer<double>::rewind(); }
                void clear(void)  { BasicVectorDataBuffer<double>::clear(); }

                bool get_data( int n, vector<double>& db, bool append );

                string get_type_srep(void) const { return "NumericVDataBuffer"; }
    ...
    // ----------------------------------------------------------------------------
    class StringVDataBuffer
        : public BasicVectorDataBuffer<string>, public StringDataBuffer {
