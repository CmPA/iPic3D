...
private:
  ...
  { return *this;}
protected:
  VarTarget() : ref_count(0) {;}   // default constructor
public:
...
public:
 ...
 // copy constructor needed so that ref_count gets incremented
  ...
  { if(p!=0) p->inc_ref_count(); }
    
// constructor from pointer
  Var( T* p_ = 0 ) : p(p_) { if(p!=0)p->inc_ref_count(); }
...
//    cout << "Var<T>& operator = ( const Var<T> &x )" << endl;
  ...
  bool is_nil(void) const { return (p==0)? true : false; }
  ...
  // static because does not need access to members of a particular
  // instance of this class
  // if T:T2 i.e. T is a derived class of T2 returns a Var<T>
  // otherwise nil
...
// dereferencing operator
  ...
  T& operator*()
  {
    if( p==0 ) throw KVFVarIsNil("_var type is nil", "Var::op*()" );
    return *p;
  }

  const T& operator*() const
  {
    if( p==0 ) throw KVFVarIsNil("_var type is nil", "Var::op*()const" );
    return *p;
  }

  T* ptr_() { return p; }

  const T* ptr_() const { return p; }
};
...
// otherwise nil
  ...
  template<class T2>
    static SeqVar<Ts> narrow( Var<T2> object )
  {
  ...
  T& operator[](int i)
  {  if( Var<Ts>::is_nil() ) throw KVFVarIsNil();
     if( i < 0 || i>= Var<Ts>::p->size() ) throw KVFVarSeqIndexError();
    return (*Var<Ts>::p)[i];
  }
