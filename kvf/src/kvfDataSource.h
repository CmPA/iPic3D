...
A KVFDataSource opens and reads a KVF file, and stores all of the
data in the file, arranged by the key name for each data item.


\note Current strategy is open, read, and close the file
in one operation, either at object construction, or by using
read_file().
...
class KVFDataSource {
  ...
  typedef enum KVFparse_state_e
    { PARSE_NIL, PARSE_NSEQ, PARSE_STRSEQ } KVFParseState;
...
public:
  ...
  void get_data_names( vector<string>& names_list,
  ...
  int num_parse_errors(void) { return _parse_errors.size(); }

  void diag_print_strseq()
    {for(int i=0;i<_strseq_buff.size();i++) cout << _strseq_buff[i] << " ";
     cout << endl; }
  void diag_print_numseq()
    {for(int i=0;i<_nseq_buff.size();i++) cout << _nseq_buff[i] << " ";
     cout << endl; }
  void diag_print_errors()
    {for(int i=0;i<_parse_errors.size();i++) cout << _parse_errors[i] << endl;}
