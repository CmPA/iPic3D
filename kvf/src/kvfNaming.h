...
public:
  ...
  string get_id(void) const { return identifier; }
  string set_id(const string& id ) { identifier = id; return identifier; }
  bool is_valid_id(void) const { return identifier != ""; }
  bool operator==( const string& a ) const { return a==identifier; }
};
...
public:
  ...
  int num_components(void) const {return components.size(); }
  ...
  string strrep( const string& separator="/" ) const { return srep();}
  string s_rep( const string& separator="/" ) const { return srep();}
...
//     since initialized from a "this" -- tobesolved?
  ...
  public:
    ...
    BindingEntry( const BindingEntry& a ) : _type(a._type), _desc(a._desc)
      {if(_type==N_CONTEXT) _nc=a._nc; else _obj=a._obj; }
    ...
    const BindingEntry& operator=( const BindingEntry& a )
      { 
      _type=a._type; _desc=a._desc;
        if(_type==N_CONTEXT) _nc=a._nc; else _obj=a._obj; return *this;}
...
//  { cout << "~NamingContext"<<endl; }
  ...
  public:
      ...
      recursive_iterator( _Ptr nc, const typename BindingMap::iterator& itr):
          _nc(nc), BindingMap::iterator(itr),_parent(0){;}
      recursive_iterator(const recursive_iterator& itr)
        :_nc(itr._nc), _parent(itr._parent),
          BindingMap::iterator(itr){;}
      ~recursive_iterator(){;}
      ...
      { // if not past-the-end, get entry type
       ...
       {   recursive_iterator tmp=*this;
            ...
            return tmp; }  
       recursive_iterator& operator--()
       { // if at beginning of a BindingMap, get previous entry off
  ...
  class const_recursive_iterator
    :public BindingMap::const_iterator,public VarTarget
  {
  protected:
  ...
  public:
      ...
      const_recursive_iterator( _CPtr nc, \
      ...
      const_recursive_iterator(const const_recursive_iterator& citr)
        : _nc(citr._nc),_parent(citr._parent),
           BindingMap::const_iterator(citr){;}
      ~const_recursive_iterator(){;}
  ...
  public:
    ...
    const_iterator(const iterator& itr):_nc(itr._nc),\
        BindingMap::const_iterator(itr){;}
    ~const_iterator(){;}
    ...
    string name() const
    { if((*this)==_nc->end())
        throw KVFNamingError("recursion off end",
          "NamingContext::const_iterator::name()");
      return (*this)->first; }

    BindingType type()const
    { if((*this)==_nc->end())
        throw KVFNamingError("recursion off end",
          "NamingContext::const_iterator::type()");
      return (*this)->second._type; }

    T object()const
    { if((*this)==_nc->end())
        throw KVFNamingError("recursion off end",
          "NamingContext::const_iterator::object()");
      if((*this)->second._type!=N_OBJECT)
        throw KVFNamingError("recursion off end",
          "NamingContext::const_iterator::object()");
      return (*this)->second._obj; }
    ...
    { if((*this)==_nc->end())
        throw KVFNamingError("recursion off end",
          "NamingContext::const_iterator::naming_context()");
      if((*this)->second._type!=N_CONTEXT)
        throw KVFNamingError("recursion off end",
          "NamingContext::const_iterator::naming_context()");
      return (*this)->second._nc; }
  ...
  public:
      ...
      matched_iterator(_Ptr nc,const typename BindingMap::iterator& itr,\
          const string& pattern=""):recursive_iterator(nc,itr),\
          _pattern(pattern)
      {
      ...
      matched_iterator(const matched_iterator& mitr):recursive_iterator(mitr),\
          _pattern(mitr._pattern){;}
...
/* Not right?
     matched_iterator& operator--()
      {
           if((*this)!=_nc->recursive_begin(_pattern))
               recursive_iterator::operator--();
           while(!recursive_iterator::match(_pattern))
               recursive_iterator::operator--();
           return *this;
...
*/
  ...
  public:
      ...
      const_matched_iterator(_CPtr nc,\
          const typename BindingMap::const_iterator& itr,\
          const string& pattern=""):const_recursive_iterator(nc,itr),\
          _pattern(pattern)
      {
          while(!const_recursive_iterator::match(pattern))
              const_recursive_iterator::operator++();
      }
      const_matched_iterator(const const_recursive_iterator& itr,\
          const string& pattern=""):const_recursive_iterator(itr),\
          _pattern(pattern)
      {
          while(!const_recursive_iterator::match(pattern))
              const_recursive_iterator::operator++();
      ...
      const_matched_iterator(const const_matched_iterator& mitr):\
          const_recursive_iterator(mitr),_pattern(mitr._pattern){;}
...
*/
  ...
  T resolve( const string& n ) { return resolve( Name(n) ); }
  ...
  void unbind( const string& n ) { unbind( Name(n) ); }
...
// simple name
    if( !n[0].is_valid_id() )
      throw KVFNameInvalid();
    typename BindingMap::const_iterator binding = \
	    bindings.find( n[0].get_id() );
...
// complex name
  ...
  {   _Var new_nc = new NamingContext(); bind_context( n, new_nc, desc ); }
  ...
  int n_components = n.num_components();
  if( n_components == 0 )
    throw KVFNameInvalid("Trying to unbind for zero n_components",
...
// complex name
  ...
  {
  bl.clear();
  for( typename BindingMap::const_iterator binding=bindings.begin();
       binding!=bindings.end(); ++binding )
    bl.push_back( Binding( Name(binding->first),
                  binding->second._type,
                  binding->second._desc ) );
  }  // end list


  void list_names_r(
   vector<string>& sl, string name_root="",
   string separator="/" )
  {
    for( typename BindingMap::const_iterator binding=bindings.begin();
         binding!=bindings.end(); ++binding )
    {
      ...
      if (binding->second._type==N_OBJECT)
          ...
          binding->second._nc->list_names_r( sl, binding_name + separator,
                                            separator  );
     }
  }
  void diag_print() 
  {
    ...
    for( itr=recursive_begin();
         itr!=recursive_end();itr++)
    { cout << itr.name();
      if( itr.type()==N_CONTEXT) cout <<"/";
