...
/*! \file
Header file for KVFException class.
...
public:
  ...
  KVFException( 
    const string& err_str, const string fn_str="", int sys_errno=0 )
    : _can_recover( true ),
      _err_str( err_str ), _fn_str( fn_str ), _type_str( "MdException::" ),
  ...
  KVFException( const KVFException& e ) :
  ...
  void diag_cout()
    {  cout << _type_str << " " << _err_str << " in function "
            << _fn_str << "\n";
       if( _sys_errno != 0 )
         cout << "sys errno: " << _sys_errno
              << " [" << _sys_err_str << "]" << "\n";
